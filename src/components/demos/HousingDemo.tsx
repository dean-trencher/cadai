...
const ElectronicHousing: React.FC<{ exploded: boolean }> = ({ exploded }) => {
  ...
  return (
    <>
      ...
      {/* Components on PCB */}
      {[[-0.8, 0.1, -0.5], [0.8, 0.1, -0.5], [0, 0.1, 0.3]].map((pos, i) => (
      ...
      {/* Screws */}
      {[[-1, -0.5, -0.7], [1, -0.5, -0.7], [-1, -0.5, 0.7], [1, -0.5, 0.7]].map((pos, i) => (
        <mesh key={i} position={pos as [number, number, number]} castShadow>
  ...
  return (
    <div className="w-full aspect-video bg-adam-darker/50 border border-white/10 rounded-lg overflow-hidden">
      ...
      <button
        ...
        className="absolute bottom-4 right-4 bg-adam-pink hover:bg-adam-pink/80 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors"
      >
